  ...
  <div class="quiz-container">
    <div class="container_1">
      <div>
        ...
        <div v-if="quizStep === 7">
          <img
            src="/images/quiz/roles_1.svg"
            alt="roles_1"
            class="roles_first"
          />
          <img
            src="/images/quiz/roles_2.svg"
            alt="roles_2"
            class="roles_second"
          />
        </div>
        <div v-if="quizStep !== 7">
          <div
            v-for="text in quiz[quizStep].answers"
            :key="text"
            class="radio-element"
          >
            <Checkbox
              v-if="quizStep === 2"
              :title="text"
              :active="!!quizANSREW[quizStep - 1].value.some((t) => t == text)"
              @click="handleSelect(text)"
            />
            <RadioButton
              v-else
              :title="text"
              :active="!!quizANSREW.filter((t) => t.value === text).length"
              @click="handleChange(text)"
            />
          </div>
        </div>
        ...
        <div v-if="quizStep === 7" class="lng_wrap">
          <div v-for="lng in quiz[quizStep].answers" :key="lng">
            <p
              class="lng_item"
              :class="{
                active: quizANSREW[quizStep - 1].value.some((t) => t === lng),
              }"
              @click="handleSelect(lng)"
            >
              + {{ lng }}
            </p>
          </div>
        ...
        <div v-if="quizStep === 8">
          ...
          <div class="form-element">
            ...
            <InputComponent
              ...
              @update:modelValue="handleFieldChange('contact_name', $event)"
            />
          </div>
          ...
          >
            ...
            />
            <div v-if="is_done" class="header-1">
      ...
      <div class="btn_wraper">
        ...
        <button
          class="default"
          :class="{ send_button: quizStep === 8 }"
          @click="quizStep === 8 ? validateForm() : nextStep()"
        >
          {{ quizStep === 8 ? 'send request' : 'next' }}
        </button>
      </div>
    </div>
    <div v-if="quizStep !== 7" class="container_2">
      <div class="logo">
        <NuxtLink to="/">
          <Logo />
        </NuxtLink>
      ...
      <div class="trust">
        <img :src="require('@/assets/illustrations/quiz-trust.svg')" />
      </div>
  ...
  data: () => ({
    ...
    quizANSREW: [
      { step: 1, value: 'Full team' },
      {
        step: 2,
        value: [
          'Developer (Software Developer, Data Scientists, DevOps or QA)',
        ],
      },
    ...
    quiz: {
      ...
      2: {
        ...
        answers: [
          'Developer (Software Developer, Data Scientists, DevOps or QA)',
          'Designer (Web, Mobile, UI/UX, Branding, Illustration)',
          'Project Manager',
          'Product Manager',
        ],
      },
      ...
      4: {
        ...
        answers: [
          ...
          'I am just exploring my options for the future',
        ],
      },
      5: {
        ...
        answers: [
          ...
          'I have a clear idea of what I want to build, but haven’t fully documented it yet',
          'I have written documentation and wireframes',
        ],
      },
      6: {
  ...
  methods: {
    ...
    validateForm() {
      ...
      this.companyNameError =
        this.company_name === '' ? 'Please, enter your company name' : null
      this.contactNameError =
        this.contact_name === '' ? 'Please, enter your contact name' : null
    ...
    handleChange(emit) {
      this.quizANSREW = this.quizANSREW.map((r) =>
        r.step === this.quizStep ? { ...r, value: emit } : r
      )
    },

    handleSelect(emit) {
      const currentQuizItem = this.quizANSREW[this.quizStep - 1]

      const isExists = currentQuizItem.value.includes(emit)
      if (isExists) {
        this.quizANSREW = this.quizANSREW.map((r) =>
          r.step === this.quizStep
            ? {
                ...r,
                value: currentQuizItem.value.filter((item) => item !== emit),
              }
            : r
        )
      } else {
        this.quizANSREW = this.quizANSREW.map((r) =>
          r.step === this.quizStep
            ? {
                ...r,
                value: [...currentQuizItem.value, emit],
              }
            : r
        )
      }
